...
4. On GitHub, [draft a new release](https://github.com/adamykchan/image-video-downloader/releases/new) with the tag `v<version>` and drop the zip into the **attach binaries** box (not the description), so the update scripts can find it. Don't rename the zip - the scripts rely on its name when GitHub limits their update checks
