...
#
# Usage (in Terminal):
#   bash update-extension.command [--force] [install folder]
# Or make it double-clickable in Finder once with: chmod +x update-extension.command

set -euo pipefail

REPO='adamykchan/image-video-downloader'
FORCE=0
INSTALL_DIR="$HOME/Extensions/image-video-downloader"
...
release=$(curl -fsSL -H 'User-Agent: image-video-downloader-updater' "https://api.github.com/repos/$REPO/releases/latest") ||
...
curl -fsSL -o "$temp/release.zip" "$asset_url" || fail "Download failed. Try again."
...
unzip -q "$temp/release.zip" -d "$temp/extracted" || [ $? -eq 1 ] || fail "Couldn't unzip $asset_name."
